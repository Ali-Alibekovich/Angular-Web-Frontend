...
export class UserService {
  ...
  doPost(user){
    ...
    this.http.post('http://localhost:8080/web_4-1.0/user-controller/check-user',user).subscribe(value => {
        if(value == false){
  ...
  doReg(user){
    user.password = MD5(user.password).toString(enc.Hex)
    this.http.post('http://localhost:8080/web_4-1.0/user-controller/add-user',user).subscribe(value => {
      localStorage.setItem('isLogged', String(value))
      if(value == false){
